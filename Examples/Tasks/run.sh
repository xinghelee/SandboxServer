#!/usr/bin/env bash
# Build, install, and launch the Tasks sample on a booted iOS Simulator, then open the debug
# console URL the app prints. The app binds localNetwork, so the URL uses a LAN IP.
set -euo pipefail
cd "$(dirname "$0")"
...
APP=$(find .build/Build/Products -name SandboxTasksDemo.app | head -1)
echo "▸ Installing…"
xcrun simctl install "$SIM_ID" "$APP"

LOG=$(mktemp)
echo "▸ Launching…"
xcrun simctl launch --console-pty "$SIM_ID" com.sandboxserver.tasks >"$LOG" 2>&1 &
LP=$!
